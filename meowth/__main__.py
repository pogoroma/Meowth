...
async def celalazio(ctx):
    """Trolling"""
    msg = '{member} è attualmente irreperibile causa impegni inderogabili. :eyes:'.format(member=ctx.author.mention)
    await ctx.channel.send(msg)
    ...
    if ctx.invoked_subcommand == None:
        ...
        if checks.check_raidchannel(ctx):
            if checks.check_raidactive(ctx):
                ...
                rc_d = guild_dict[guild.id]['raidchannel_dict'][channel.id]
                if _(" 0 interested!") not in await _interest(ctx):
                    listmsg += ('\n' + bulletpoint) + (await _interest(ctx))
                if _(" 0 on the way!") not in await _otw(ctx):
                    listmsg += ('\n' + bulletpoint) + (await _otw(ctx))
                if _(" 0 waiting at the raid!") not in await _waiting(ctx):
                    listmsg += ('\n' + bulletpoint) + (await _waiting(ctx))
                if _(" 0 in the lobby!") not in await _lobbylist(ctx):
...
async def mystic(ctx, tag=False):
    ...
    if checks.check_raidchannel(ctx):
        if checks.check_raidactive(ctx):
            ...
            starttime = guild_dict[guild.id]['raidchannel_dict'][channel.id].get('starttime',None)
            rc_d = guild_dict[guild.id]['raidchannel_dict'][channel.id]
            if _(" 0 interested!") not in await _interest(ctx, tag, team="mystic"):
                listmsg += ('\n' + bulletpoint) + (await _interest(ctx, tag=False, team="mystic"))
            if _(" 0 on the way!") not in await _otw(ctx, tag, team="mystic"):
                listmsg += ('\n' + bulletpoint) + (await _otw(ctx, tag, team="mystic"))
            if _(" 0 waiting at the raid!") not in await _waiting(ctx, tag, team="mystic"):
                listmsg += ('\n' + bulletpoint) + (await _waiting(ctx, tag, team="mystic"))
            if _(" 0 in the lobby!") not in await _lobbylist(ctx, tag, team="mystic"):
...
async def valor(ctx, tag=False):
    ...
    if checks.check_raidchannel(ctx):
        if checks.check_raidactive(ctx):
            ...
            starttime = guild_dict[guild.id]['raidchannel_dict'][channel.id].get('starttime',None)
            rc_d = guild_dict[guild.id]['raidchannel_dict'][channel.id]
            if _(" 0 interested!") not in await _interest(ctx, tag, team="valor"):
                listmsg += ('\n' + bulletpoint) + (await _interest(ctx, tag, team="valor"))
            if _(" 0 on the way!") not in await _otw(ctx, tag, team="valor"):
                listmsg += ('\n' + bulletpoint) + (await _otw(ctx, tag, team="valor"))
            if _(" 0 waiting at the raid!") not in await _waiting(ctx, tag, team="valor"):
                listmsg += ('\n' + bulletpoint) + (await _waiting(ctx, tag, team="valor"))
            if _(" 0 in the lobby!") not in await _lobbylist(ctx, tag, team="valor"):
...
async def instinct(ctx, tag=False):
    ...
    if checks.check_raidchannel(ctx):
        if checks.check_raidactive(ctx):
            ...
            starttime = guild_dict[guild.id]['raidchannel_dict'][channel.id].get('starttime',None)
            rc_d = guild_dict[guild.id]['raidchannel_dict'][channel.id]
            if _(" 0 interested!") not in await _interest(ctx, tag, team="instinct"):
                listmsg += ('\n' + bulletpoint) + (await _interest(ctx, tag, team="instinct"))
            if _(" 0 on the way!") not in await _otw(ctx, tag, team="instinct"):
                listmsg += ('\n' + bulletpoint) + (await _otw(ctx, tag, team="instinct"))
            if _(" 0 waiting at the raid!") not in await _waiting(ctx, tag, team="instinct"):
                listmsg += ('\n' + bulletpoint) + (await _waiting(ctx, tag, team="instinct"))
            if _(" 0 in the lobby!") not in await _lobbylist(ctx, tag, team="instinct"):
